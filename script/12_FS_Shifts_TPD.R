...
TPDs_fish <- TPDsMean( # long recommended skip
...
core_099  <- imageTPD_core(TPDc_use, thresholdPlot = 0.999)
...
# ---- Palette: black -> yellow -> red (low to high) ----
deficit_palette <- function(n) {
  grDevices::colorRampPalette(
    c("#0F1108", "#F5E0B7", "#A44200"),  # noir chaud -> ambre -> terre cuite
    space = "Lab"
  )(n)
}
...
plot_one_panel_deficit <- function(catg,
  ...
  # Palette
  ColorRamp <- deficit_palette(ncol)
...
plot_panel_deficit <- function(out_pdf = "figures/Figure_FunctionalShifts_clean_v2.pdf",
...
plot_individual_deficit <- function(out_dir = "figures/individual_panels",
  ...
  for (catg in usages_order) {
    # ---- PNG ----
    png(
      filename = file.path(out_dir, paste0("Deficit_PC1PC2", gsub(" ", "_", catg), ".png")),
    ...
    # ---- PDF ----
    pdf(
      file = file.path(out_dir, paste0("Deficit_PC1PC2", gsub(" ", "_", catg), ".pdf")),
...
TPD_2D <- TPDsMean( # long recommended skip
  species = rownames(pca_trait$traits_scores),
  means   = pca_trait$traits_scores[, c(3, 4)],
  sds     = matrix(
    rep(sd_traits, nrow(pca_trait$traits_scores)),
    byrow = TRUE, ncol = 2
  ),
...
saveRDS(TPD_2D, "output/TPD_2D_PC3PC4.rds")
TPD_2D <- readRDS("output/TPD_2D_PC3PC4.rds")
...
# ---- Single TPDc call for ALL + all usages ----
TPDc_use <- TPD::TPDc(TPDs = TPD_2D, sampUnit = comm) # long
...
eval_grid <- TPDc_use$data$evaluation_grid
Comp.3Vec <- unique(eval_grid[, 1])
Comp.4Vec <- unique(eval_grid[, 2])
...
get_pc_labels <- function(pca_obj) {
  ...
  if (!is.null(var)) {
    ...
    xlab <- sprintf("PC 3 (%.1f%%)", v1)
    ylab <- sprintf("PC 4 (%.1f%%)", v2)
  } else {
    xlab <- "PC 3"
    ylab <- "PC 4"
  }
...
# Plot
# ------------------------------------------------------------------------------
pad_x3 <- diff(range(eval_grid[, 1])) * 0.05
pad_y4 <- diff(range(eval_grid[, 2])) * 0.05

x_limits_34 <- range(eval_grid[, 1]) + c(-pad_x3, pad_x3)
y_limits_34 <- range(eval_grid[, 2]) + c(-pad_y4, pad_y4)
...
plot_individual_deficit <- function(out_dir = "figures/individual_panels_34",
  ...
  for (catg in usages_order) {
    ...
    png(
      filename = file.path(out_dir, paste0("Deficit_PC3PC4", gsub(" ", "_", catg), ".png")),
      width = png_px, height = png_px, res = png_res, type = "cairo"
    )
    on.exit(dev.off(), add = TRUE)
    par(mar = c(5.2, 5.2, 2.5, 0.8))
    ...
    # PDF (square)
    pdf(
      file = file.path(out_dir, paste0("Deficit_PC3PC4", gsub(" ", "_", catg), ".pdf")),
      width = 7, height = 7, pointsize = 14
    )
    on.exit(dev.off(), add = TRUE)
    par(mar = c(5.2, 5.2, 2.5, 0.8))
