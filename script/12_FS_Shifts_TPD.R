...
# Functional deficit maps in morphospace (TPD-based occupancy deficit).
#
# To quantify local under-representation of each usage relative to the global
# pool, we compute in each grid cell the number of species occupying that cell
# for ALL and for each usage. We then derive a bounded deficit surface
# Deficit(cell) = (N_ALL(cell) - N_Usage(cell)) / N_ALL(cell) in [0, 1].
#
# A deficit of 0 means the usage covers that region of morphospace as densely as
# the global pool; a deficit of 1 means no used species occupies it at all.
#
# Outputs: figures/Figure_FunctionalShifts_clean.{pdf,png} and one square panel
# per usage in figures/individual_panels/.
...
# Converts each community density into a map of cumulative percentiles; cells
# above thresholdPlot are set to NA, which defines the visible core.
...
# Number of species whose density is non-zero in each cell, per community.
...
# One row per community: "ALL" holds every species and serves as the reference.
comm <- matrix(
  0,
...
# ---- Single TPDc call for ALL + all usages  [LONG] ----
# LONG: aggregates the density of 8,970 species over a 200 x 200 grid, for the
# six communities at once. Everything below is fast in comparison.
#
# First run: leave the two lines below active, they write the result to output/.
# Later runs: comment them out and uncomment the readRDS instead.
TPDc_use <- TPD::TPDc(TPDs = TPDs_fish, sampUnit = comm)
saveRDS(TPDc_use, "output/TPDc_use_2D.rds")

# TPDc_use <- readRDS("output/TPDc_use_2D.rds")
...
# ---- One panel ----
# Draws the deficit surface of a single usage, masked to the global 99% core and
# outlined by the global support contour.
...
# Five panels in a row plus a vertical colour bar, exported as PDF and PNG.
...
# ---- Individual square high-resolution exports (PNG + PDF) ----
